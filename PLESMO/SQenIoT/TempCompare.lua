...
function TempCompare.compareTemperature(...)
	...
	-- arg.n is the real size
		for i = 1,argT.n do
			TempCompare.StallmanAvg = TempCompare.StallmanAvg +engine.getMeasurementValue(argT[i], 'Temp');
		end
		...
		--Get avg temp on whole floor
			if #TempCompare.resources ~= 0 then	
				for i = 1, #TempCompare.resources do
					TempCompare.FloorAvg = TempCompare.FloorAvg +engine.getMeasurementValue(TempCompare.resources[i], 'Temp');
				end
...
function TempCompare.query(...)	
	return engine.query('TempCompare', ...) 	
end

-- Delegate function to simplify syntax 
function TempCompare.subscribe(...)	
	engine.subscribe('TempCompare', ...) 	
end

-- Delegate function to simplify syntax 
function TempCompare.unSubscribe(...)	
	engine.unSubscribe('TempCompare', ...) 	
end
